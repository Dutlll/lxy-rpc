...
public class RpcClientManager {
    ...
    public static void main(String[] args) {
        try {
            ...
            service = getProxyService(HelloService.class);
            System.out.println(service.sayHello("zhangsan22"));
...
//                    //            // 3. 准备一个空 Promise 对象，来接收结果             指定 promise 对象异步接收结果线程
//                    log.debug("我放入了数据，目前等待响应");                    log.debug("我放入了数据，目前等待响应");
//                    log.debug("我放入了数据，目前等待响应");
//                    log.debug("我放入了数据，目前等待响应");
//                    log.debug("我放入了数据，目前等待响应");
//                    log.debug("我放入了数据，目前等待响应");
//
...
//                }
//            };
//            final ChannelFuture channelFuture = getChannel().writeAndFlush(msg)
//                    .addListener(
//                            channelFutureListener
//                    );
//            channelFutureListener();
//            channelFuture.sync();
//            channelFutureListener.wait();
//            return RpcResponseMessageHandler.PROMISES.get(sequenceId).getNow();
//            // 3. 准备一个空 Promise 对象，来接收结果             指定 promise 对象异步接收结果线程
//            DefaultPromise<Object> promise = new DefaultPromise<>(getChannel().eventLoop());
//            RpcResponseMessageHandler.PROMISES.put(sequenceId, promise);
//
...
//            return channelFuture.toString();
    ...
    private static void initChannel() {
        ...
        try {
            channel = bootstrap.connect("localhost", 10003).sync().channel();
