...
import com.lxy.rpc.client.callback.ParalCallbackHandler;
...
@Component
public class ParalRPCProxyer implements InitializingBean{

    @Autowired
    private RPCRequestPolicyBuilder rpcRequestPolicyBuilder;

    @Autowired
    private ParalCallbackHandler paralCallbackHandler;
...
//            final AbstractTransmitter transmitter = longConnectTransmitterPool.getTransmitter(msg);
            ...
            //4,根据处理策略，处理服务端的响应
            try {
                final RPCRequestCallback rpcRequestCallback = paralCallbackHandler.rpcCallback(send);
//                rpcRequestCallback.getId();//
                return rpcRequestCallback;
